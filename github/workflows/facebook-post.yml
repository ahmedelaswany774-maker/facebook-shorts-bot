...
End-to-end generator + publisher for a Facebook-ready short vertical video +
caption. Pipeline: Groq (script+caption) -> gTTS (voiceover) ->
Pollinations.ai (images) -> FFmpeg (assemble) -> Facebook Graph API (publish
to a Page).

Requirements (all free / zero-cost):
    pip install requests gTTS --break-system-packages
    ffmpeg must be installed and on PATH

Env vars required:
    GROQ_API_KEY            - Groq API key for script/caption generation
    FB_PAGE_ACCESS_TOKEN    - Long-lived Page Access Token (Graph API)
    FB_PAGE_ID              - Numeric ID of the Facebook Page to publish to

If FB_PAGE_ACCESS_TOKEN / FB_PAGE_ID are not set, the script falls back to
local-only mode (generates the files but does not publish), so it's safe to
run without them for testing.
...
def call_groq(topic: str | None) -> dict:
    ...
    topic_line = f"الموضوع المطلوب: {topic}" if topic else (
        "اختر موضوعًا عامًا مثيرًا للاهتمام (قصة قصيرة أصلية، معلومة غريبة، أو موقف إنساني)."
    )

    prompt = textwrap.dedent(f"""
    أنت كاتب محتوى عربي لفيديوهات على فيسبوك مدتها حوالي 3 دقائق (حوالي 420-480 كلمة نطق).
    {topic_line}
    اكتب قصة أصلية 100% من تأليفك (ممنوع نسخ أي نص من مصادر خارجية)، بحيث تكون مقسّمة
    لمقدمة تشد الانتباه، وسط فيه تطور وتفاصيل، وخاتمة قوية.
...
def post_to_facebook_page(video_path: Path, caption: str) -> dict:
    """
    Upload and publish a video to a Facebook Page via the Graph API.
    Requires FB_PAGE_ACCESS_TOKEN + FB_PAGE_ID env vars (Page-level token
    with pages_manage_posts + pages_read_engagement scopes).
    """
    if not FB_PAGE_ACCESS_TOKEN or not FB_PAGE_ID:
        raise RuntimeError("FB_PAGE_ACCESS_TOKEN / FB_PAGE_ID not set.")
...
def main():
    ...
    if FB_PAGE_ACCESS_TOKEN and FB_PAGE_ID:
        ...
        try:
            result = post_to_facebook_page(video_path, data["caption"])
            print(f"✅ اتنشر بنجاح. Video ID: {result.get('id')}")
        except Exception as exc:
            print(f"❌ فشل النشر التلقائي: {exc}")
            print("الفيديو والكابشن لسه محفوظين محليًا، تقدر تنشرهم بإيدك.")
            sys.exit(1)
    else:
        print("\n(FB_PAGE_ACCESS_TOKEN / FB_PAGE_ID مش متضبطين — النشر التلقائي متعطّل، لازم تنشر بإيدك.)")
