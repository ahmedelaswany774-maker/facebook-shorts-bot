...
def generate_images(script_text: str, out_dir: Path, num_images: int = 12) -> list[Path]:
    ...
    Resilient to Pollinations.ai transient failures (500s, timeouts):
    retries each image a few times with backoff, and if an image keeps
    failing, skips it rather than crashing the whole run -- as long as
    we end up with at least one image, we can still assemble a video.
    """
    ...
    for i, beat in enumerate(beats):
        ...
        for attempt in range(3):
            try:
                ...
                print(f"  ⚠️ فشلت محاولة {attempt + 1}/3 للصورة {i}: {e}")
                if attempt < 2:
                    time.sleep(3 * (attempt + 1))
                else:
                    print(f"  ❌ اتخطينا الصورة {i} بعد 3 محاولات فاشلة.")
        time.sleep(1)  # be polite to the free endpoint

    if not paths:
        raise RuntimeError("فشل توليد كل الصور من Pollinations.ai -- مفيش صور نبني بيها الفيديو.")
    return paths
...
def main():
    ...
    if FB_PAGE_ACCESS_TOKEN and FB_PAGE_ID:
        ...
        try:
            ...
            # لا نوقف السكريبت بالكامل لو النشر التلقائي فشل (مثلاً بانتظار
            # Business Verification) -- الفيديو والكابشن جاهزين للنشر اليدوي
            # على أي حال، وده أهم حاجة.
            print(f"\n⚠️ النشر التلقائي فشل مؤقتًا: {e}")
            print("الفيديو والكابشن لسه جاهزين تمامًا للنشر اليدوي من الـ Artifact.")
    else:
        print(
