...
export async function runAiPipeline(): Promise<PipelineContext> {
  console.log("1/3 Idea: جاري اختيار الفكرة...");
  const idea = await getNextIdea();
  console.log(`   -> ${idea.topic} (source: ${idea.source})`);

  console.log("2/3 Research: جاري تجميع الزاوية والحقائق...");
  const researchResult = await research(idea);

  console.log("3/3 Hook: جاري كتابة الجملة الافتتاحية...");
  ...
  if (idea.notionPageId) {
    ...
    // (Script -> ... -> Publish) aren't built yet.
    await updatePage(idea.notionPageId, {
      Caption: props.richText(hook.text),
    });
  }

  const context: PipelineContext = { idea, research: researchResult, hook };
  ...
  console.log(`\n✅ Idea/Research/Hook جاهزين -- محفوظين في ${outDir}/pipeline.json`);
  console.log(
    "⏭️  الخطوة الجاية (Script Generator - Phase 4) لسه محتاجة تتبني.\n" +
      "   شوف TODO.md."
  );
